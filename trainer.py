...
class Trainer():
    def __init__(self, config):
        ...
        all_3d_tracks, drive_list, self.track_list_train = parse_trajectories.get_all_3d_tracks(train=True, draw_tracks=False,
                                                                                     plot_3d=False)
        all_3d_tracks, drive_list, self.track_list_test = parse_trajectories.get_all_3d_tracks(train=False, draw_tracks=False,
                                                                                     plot_3d=False)

        print('creating dataset...')
        self.data_train = dataset.TrackDataset(self.track_list_train, num_istances=config.past_len, num_labels=config.future_len)
        self.train_loader = DataLoader(self.data_train,
                                       batch_size=config.batch_size,
                                       num_workers=1,
                                       shuffle=True
                                       )
        self.data_test = dataset.TrackDataset(self.track_list_test, num_istances=config.past_len, num_labels=config.future_len)
    ...
    def evaluate(self, loader, epoch=0):
        ...
        for step, (index, story, future, videos, vehicles) in enumerate(loader):
    ...
    def _train_single_epoch(self, epoch):
        config = self.config
        for step, (index, story, future, videos, vehicles) in enumerate(self.train_loader):
            self.iterations +=1
            story = Variable(story)
            future = Variable(future)
            if config.cuda:
